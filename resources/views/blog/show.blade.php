<x-layouts.main :title="$post->meta_title ?: $post->title" :description="$post->meta_description ?: $post->excerpt">
    <article class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <!-- Back Link -->
        <a href="{{ route('blog.index') }}" class="inline-flex items-center text-sm text-slate-600 dark:text-slate-400 hover:text-primary-600 dark:hover:text-primary-400 mb-8">
        ...
        <header class="mb-8">
            <div class="flex items-center gap-2 mb-4">
                ...
                <a href="{{ route('blog.category', $category->slug) }}" class="px-3 py-1 text-xs font-medium bg-primary-100 dark:bg-primary-900/30 text-primary-700 dark:text-primary-400 rounded-full hover:bg-primary-200 transition">
                    {{ $category->name }}
                </a>
                @endforeach
            </div>
            <h1 class="text-3xl sm:text-4xl font-bold text-slate-900 dark:text-white leading-tight">{{ $post->title }}</h1>
        ...
        <figure class="mb-10">
            <img src="{{ Storage::url($post->featured_image) }}" alt="{{ $post->title }}" class="w-full rounded-xl">
        </figure>
        @endif
        ...
        <div class="mt-8 pt-8 border-t border-slate-200 dark:border-dark-border">
            <div class="flex items-center gap-4">
                ...
                <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(request()->url()) }}" target="_blank" class="p-2 bg-slate-100 dark:bg-dark-soft rounded-full hover:bg-primary-100 dark:hover:bg-primary-900/30 transition">
                    <svg class="w-5 h-5 text-slate-600 dark:text-slate-400" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                    </svg>
                </a>
                <a href="https://wa.me/?text={{ urlencode($post->title . ' ' . request()->url()) }}" target="_blank" class="p-2 bg-slate-100 dark:bg-dark-soft rounded-full hover:bg-primary-100 dark:hover:bg-primary-900/30 transition">
                    <svg class="w-5 h-5 text-slate-600 dark:text-slate-400" fill="currentColor" viewBox="0 0 24 24">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <a href="{{ route('blog.show', $related->slug) }}" class="group block bg-white dark:bg-dark-elevated rounded-xl border border-slate-200 dark:border-dark-border overflow-hidden hover:shadow-lg transition">
                    ...
                    @endif
                    <div class="p-4">
                        <h3 class="font-semibold text-slate-900 dark:text-white group-hover:text-primary-600 dark:group-hover:text-primary-400 transition line-clamp-2">{{ $related->title }}</h3>
                        <p class="mt-2 text-xs text-slate-500 dark:text-slate-400">{{ $related->published_at->format('M d, Y') }}</p>
                    </div>
                </a>
                @endforeach
            </div>
